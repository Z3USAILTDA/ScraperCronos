...
public static class Utils
{
    ...
    public static void WriteDiagFile(string awb, string suffix, string? content)
    {
        try
        {
            var dir = Path.Combine(AppContext.BaseDirectory, "diag", awb);
            Directory.CreateDirectory(dir);

            var safeSuffix = suffix.Replace(":", "_").Replace("/", "_").Replace("\\", "_");
            var path = Path.Combine(dir, safeSuffix);

            File.WriteAllText(path, content ?? "", Encoding.UTF8);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[DIAG] Falha ao gravar arquivo de diagnóstico: {ex.Message}");
        }
    }
}